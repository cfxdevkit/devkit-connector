...
import type { ConfluxNodeManager } from './services/conflux-node-manager';

const app: Express = express();
const PORT = process.env.PORT || 3001;

// Global node manager instance
const globalNodeManager: ConfluxNodeManager | null = null;
...
const limiter = rateLimit({
  ...
  handler: (req: unknown, res) => {
    const retryAfter = Math.round(
      ((req as any).rateLimit.resetTime - Date.now()) / 1000
    );
    console.log(
      `🚫 Rate limit hit for ${(req as any).ip} on ${(req as any).path} - retry in ${retryAfter}s`
    );
...
async function initializeServer() {
  try {
    ...
    (global as Record<string, unknown>).demoWalletId = walletData.walletId;
  } catch (error) {
    console.log(
