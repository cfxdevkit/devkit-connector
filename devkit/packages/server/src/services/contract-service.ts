...
export class ContractService {
  ...
  constructor(config: ContractServiceConfig) {
    ...
    try {
      ...
      if (fs.existsSync(configPath)) {
        ...
        console.log(
          `📋 Loaded contract config with ${Object.keys(this.contractConfig.contracts).length} contracts`
        );
      } else {
        ...
        for (const file of files) {
          if (file.endsWith('.json')) {
            ...
            if (deployment.network && deployment.address) {
              ...
              console.log(
                `📄 Loaded legacy ${key} deployment: ${deployment.address}`
              );
            }
          }
        }
    ...
    try {
      for (const [contractName, contract] of Object.entries(
        ...
        const deploymentResult: DeploymentResult = {
          ...
          timestamp:
            (contract as any).metadata?.timestamp || new Date().toISOString(),
        ...
        console.log(
          `🔧 Loaded config ${contractName}: ${(contract as any).address} (${(contract as any).source})`
        );
      }
    } catch (error) {
      console.warn('⚠️ Failed to load contract config:', error);
  ...
  async getContractStatus(): Promise<ContractStatus> {
    ...
    const delegationDeployment =
      this.deployments.get('DelegationManager') ||
      this.deployments.get('delegation') ||
      this.deployments.get('DelegationManagerModule#DelegationManager');

    console.log(
      `📊 Contract status - Counter: ${counterDeployment?.address}, Delegation: ${delegationDeployment?.address}`
    );
