...
interface HardhatRuntimeEnvironment {
  ignition?: {
    deploy: (modulePath: string, options?: unknown) => Promise<unknown>;
  };
  network: {
    name: string;
    config: unknown;
  ...
  run: (task: string, ...args: unknown[]) => Promise<unknown>;
}
...
class HardhatService {
  ...
  private async initializeHRE(): Promise<void> {
    ...
    try {
      ...
      this.hre = hardhat.default as HardhatRuntimeEnvironment;
