...
import { type ethers, HDNodeWallet, Mnemonic } from 'ethers';
import type { ChainType, WalletOperation } from '../types/wallet';
import type { ConfluxNodeManager, WalletInfo } from './conflux-node-manager';
...
export class ServerWalletService {
  ...
  async signTransaction(
    ...
    if (chainType === 'eSpace') {
      return await wallet.signTransaction(transaction as any);
    } else {
  ...
  validateWalletOperation(
    ...
    if (config.maxTransactionValue && (operation.payload as any).value) {
      const maxValue = BigInt(config.maxTransactionValue);
      const operationValue = BigInt((operation.payload as any).value);
