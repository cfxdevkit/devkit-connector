...
import { NodeConfig, WalletInfo } from "./types.js";
...
export class WalletManager {
  ...
  private coreClient?: any; // cive client
  private evmClient?: any; // viem client
          ...
          if (this.wallets.length > 1) {
            ...
            for (let i = 1; i < this.wallets.length; i++) {
              ...
              try {
                ...
                await this.evmClient.waitForTransactionReceipt({
                  hash: txHash,
                });
  ...
  private async updateBalances(): Promise<void> {
    ...
    for (const wallet of this.wallets) {
      try {
        const balance = await this.coreClient.cfx.getBalance(wallet.address);
        wallet.balance = balance.toString();
      } catch (error) {
        console.log(
          `⚠️  Failed to get balance for wallet ${wallet.address}: ${error}`
        );
      }
    }
  }
   ...
   */
  setClients(coreClient: any, evmClient: any): void {
