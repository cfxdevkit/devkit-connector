...
router.post('/revokeDelegation', async (req: Request, res: Response) => {
  try {
...
router.get('/getDelegation/:address', async (req: Request, res: Response) => {
  try {
    ...
    const result = await contractService.callEspaceContract('getDelegation', [address]);
    res.json(result);
  } catch (error) {
    res.status(500).json({ 
      success: false, 
...
router.post('/counter/operation', async (req: Request, res: Response) => {
  try {
    ...
    if (operation === 'batchAdd' || operation === 'batchSubtract') {
      ...
      const result = await contractService.performCounterOperation(validated.operation, undefined, validated.values);
      res.json(result);
    } else {
      const validated = counterOperationSchema.parse({ operation, value });
      const result = await contractService.performCounterOperation(validated.operation, validated.value);
      res.json(result);
    }
  } catch (error) {
    if (error instanceof z.ZodError) {
